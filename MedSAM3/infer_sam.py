...
class SAM3LoRAInference:
    ...
    def __init__(
        ...
        Args:
            ...
            wbc_mode: Keep only the single best (highest-confidence + most-central) mask
                      that covers at least min_mask_area_frac of the image. Designed for
                      pre-cropped WBC images where exactly one WBC is expected per image.
            min_mask_area_frac: Minimum mask area as a fraction of total image pixels
                                (default 0.02 = 2%). Discards tiny fragment detections.
            fallback_threshold: If no detections survive detection_threshold, retry at
                                this lower threshold. None disables fallback (default).
        """
        # Load config
    ...
    def predict(
        ...
        for query_idx, prompt in enumerate(text_prompts):
            ...
            # Filter by threshold; fall back to a lower threshold if nothing survives
            keep = scores > self.detection_threshold
            num_keep = keep.sum().item()
            active_threshold = self.detection_threshold
            if num_keep == 0 and self.fallback_threshold is not None:
                keep = scores > self.fallback_threshold
                num_keep = keep.sum().item()
                active_threshold = self.fallback_threshold
                if num_keep > 0:
                    print(f"   '{prompt}': fallback threshold {self.fallback_threshold} used")
            ...
            if num_keep > 0:
                ...
                if pred_masks is not None:
                    ...
                    if self.fill_holes:
                        ...
                        masks_np = np.stack([binary_fill_holes(m) for m in masks_np])
                else:
                    masks_np = None

                # WBC mode: keep only the single best detection near the image center
                # that meets a minimum area requirement.
                if self.wbc_mode and num_keep > 0 and masks_np is not None:
                    total_pixels = orig_h * orig_w
                    cx_img, cy_img = orig_w / 2.0, orig_h / 2.0
                    best_idx = None
                    best_score = -1.0
                    for mi in range(num_keep):
                        mask = masks_np[mi]
                        area = mask.sum()
                        # Discard masks smaller than min_mask_area_frac of image
                        if area < self.min_mask_area_frac * total_pixels:
                            continue
                        # Compute centroid proximity to image center (normalized 0-1)
                        ys, xs = np.where(mask)
                        if len(xs) == 0:
                            continue
                        cx_m, cy_m = xs.mean(), ys.mean()
                        dist = ((cx_m - cx_img) ** 2 + (cy_m - cy_img) ** 2) ** 0.5
                        max_dist = ((orig_w / 2) ** 2 + (orig_h / 2) ** 2) ** 0.5
                        proximity = 1.0 - dist / max_dist  # 1 = dead center, 0 = corner
                        # Combined score: confidence + proximity bonus
                        combined = kept_scores[mi].item() + 0.5 * proximity
                        if combined > best_score:
                            best_score = combined
                            best_idx = mi
                    if best_idx is not None:
                        boxes_xyxy  = boxes_xyxy[best_idx:best_idx+1]
                        kept_scores = kept_scores[best_idx:best_idx+1]
                        masks_np    = masks_np[best_idx:best_idx+1]
                        num_keep    = 1
                    else:
                        # All candidates failed min-area filter — report 0 detections
                        boxes_xyxy  = None
                        kept_scores = None
                        masks_np    = None
                        num_keep    = 0
                ...
                if num_keep > 0:
                    results[query_idx] = {
                        'prompt': prompt,
                        'boxes': boxes_xyxy.cpu().numpy() if isinstance(boxes_xyxy, torch.Tensor) else boxes_xyxy,
                        'scores': kept_scores.cpu().numpy() if isinstance(kept_scores, torch.Tensor) else kept_scores,
                        'masks': masks_np,
                        'num_detections': num_keep
                    }
                    max_score = (kept_scores.max().item() if isinstance(kept_scores, torch.Tensor)
                                 else kept_scores.max())
                    print(f"   '{prompt}': {num_keep} detections after NMS (max score: {max_score:.3f})")
                else:
                    results[query_idx] = {
                        'prompt': prompt,
                        'boxes': None,
                    ...
                    print(f"   '{prompt}': 0 detections (filtered by wbc-mode)")
            else:
                results[query_idx] = {
                    'prompt': prompt,
                    'boxes': None,
    ...
    def save_masked_image(self, results: dict, output_path: str) -> None:
        """
        Save the original image with only the masked region visible.
        Pixels outside all detected masks become transparent (alpha = 0).
        The image is saved at its original resolution with no rescaling.
        """
        pil_image = results['_image']
        orig_w, orig_h = pil_image.size
        ...
        # Union of every mask across all prompts
        ...
        rgba[:, :, 3] = np.where(combined_mask, 255, 0)

        from PIL import Image as _PIL
        _PIL.fromarray(rgba).save(output_path)

        if combined_mask.any():
            print(f"\n✅ Saved masked image to {output_path}")
            print(f"   Mask coverage: {100 * combined_mask.mean():.1f}% of image")
        else:
            print(f"\n⚠️  No detections — saved fully transparent image to {output_path}")
...
def main():
    ...
    parser.add_argument(
        "--wbc-mode",
        action="store_true",
        help="WBC mode: keep only the single best (most-central, min-area) detection. "
             "Designed for pre-cropped images where exactly one WBC is expected."
    )
    parser.add_argument(
        "--min-mask-area-frac",
        type=float,
        default=0.02,
        help="Minimum mask area as fraction of image pixels (default: 0.02). "
             "Discards tiny fragment detections. Only used with --wbc-mode."
    )
    parser.add_argument(
        "--fallback-threshold",
        type=float,
        default=None,
        help="If no detections survive --threshold, retry at this lower value "
             "(e.g. 0.2). Helps morphologically distinct cell types like EBO/MYO/LYA."
    )
    parser.add_argument(
        "--masked-output",
        action="store_true",
        help="Save the original image with only the masked region visible "
             "(transparent background) instead of a visualization overlay."
    )
    parser.add_argument(
        "--fill-holes",
        action="store_true",
        help="Fill interior holes in predicted masks using binary_fill_holes. "
             "Helps when the model predicts a ring/crescent instead of a solid blob "
             "(e.g. high nucleus-cytoplasm contrast cells like erythroblasts, granulocytes)."
    )
    ...
    if args.masked_output:
        inferencer.save_masked_image(results, args.output)
    else:
